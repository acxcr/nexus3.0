...
while true; do
    ...
    echo "[$(date)] 本轮休眠 \$SLEEP_SECONDS 秒..." | tee -a "\${DAEMON_LOG_FILE}"
    sleep \$SLEEP_SECONDS
...
function restart_single_instance() {
    local key_to_restart=$1
    echo "▶️ 正在销毁并重建实例 ${key_to_restart}..."
    local instance_data=$(jq ".\"$key_to_restart\"" "$CONFIG_FILE")
    local node_id=$(echo "$instance_data" | jq -r '.node_id')
    local proxy_addr=$(echo "$instance_data" | jq -r '.proxy_address')
    local instance_num=$(echo "$key_to_restart" | sed 's/nexus-node-//')
    ...
    docker rm -f "$key_to_restart" &>/dev/null || true
    docker run -d \
        --name "$key_to_restart" \
        -e NODE_ID="$node_id" \
        -e PROXY_ADDR="$proxy_addr" \
        ...
        -v "$log_file":"$log_file" \
        "$IMAGE_NAME"
    echo "✅ 实例 $key_to_restart 已使用ID [$node_id] 重启。"
}
    ...
    for i in $(seq 1 "$group_count"); do
        ...
        for node_id in "${id_pool[@]}"; do
            ...
            current_config=$(echo "$current_config" | jq \
                ...
                --arg group_name "group-${group_id_counter}" \
    ...
    if [[ "$selected_num" =~ ^[0-9]+$ ]]; then
        ...
        case "$action" in
            1) 
                ...
                ;;
            2)
                restart_single_instance "$selected_key"
                ;;
